'use strict'
const Servicio = use('App/Models/ServiciosOferta')
class ServiciosOfertaController {
  ...
  async update ({params, request, response}) {
    ...
    var servicio = await Servicio.query().select('*').where('lavanderia_id','=',params.id).fetch()
    servicio = servicio.toJSON();
    const servicioC = await Servicio.find(servicio.id)
    if (!servicioC) {
      return response.status(404).json({data: 'Resource not found'})
    }
    servicioC.servicio = servicioInfo.servicio
...
module.exports = ServiciosOfertaController
